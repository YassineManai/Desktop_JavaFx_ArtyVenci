...
public class PostTemplateMembreOwnerController {
...
//            post_label_id.getScene().setRoot(root);
    ...
    ///////////EDIT LOGIC
    @FXML
    private ImageView Edit_butt;
    @FXML
    private Button EditButt;
    @FXML
    void EditPost(ActionEvent event) {
        try{
            Parent root = loadRootEdit();
            EditButt.getScene().setRoot(root);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    private Parent loadRootEdit() throws Exception {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/ForumPages/Member/EditPostPageMembre.fxml"));
        EditPostPageMembreController controller = new EditPostPageMembreController();
        loader.setController(controller);
        controller.setData(SF.getbyid(post.getId_forum()),post); // Add data to the controller
